...
const getImcClassificacao = (imc: number) => {
  if (imc < 18.5) return { classificacao: 'Baixo peso', cor: 'var(--color-primary-blue)' };
  if (imc < 25) return { classificacao: 'Peso normal', cor: 'var(--color-primary-green)' };
  if (imc < 30) return { classificacao: 'Sobrepeso', cor: 'var(--color-primary-blue)' };
  return { classificacao: 'Obesidade', cor: 'var(--color-primary-blue)' };
};

const getMmiClassificacao = (tempo: number, idade: number | null) => {
  if (!idade) return { classificacao: 'Indefinido', cor: 'gray' };
  if (idade < 60) {
    if (tempo < 12) return { classificacao: 'Excelente', cor: 'var(--color-primary-green)' };
    if (tempo < 15) return { classificacao: 'Bom', cor: 'var(--color-primary-green)' };
    if (tempo < 18) return { classificacao: 'Regular', cor: 'var(--color-primary-blue)' };
    return { classificacao: 'Fraco', cor: 'var(--color-primary-blue)' };
  } else {
    if (tempo < 14) return { classificacao: 'Excelente', cor: 'var(--color-primary-green)' };
    if (tempo < 17) return { classificacao: 'Bom', cor: 'var(--color-primary-green)' };
    if (tempo < 21) return { classificacao: 'Regular', cor: 'var(--color-primary-blue)' };
    return { classificacao: 'Fraco', cor: 'var(--color-primary-blue)' };
  }
};

const getPreensaoClassificacao = (forca: number, mao: 'direita' | 'esquerda') => {
  if (forca >= 30) return { classificacao: 'Excelente', cor: 'var(--color-primary-green)' };
  if (forca >= 20) return { classificacao: 'Bom', cor: 'var(--color-primary-green)' };
  if (forca >= 10) return { classificacao: 'Regular', cor: 'var(--color-primary-blue)' };
  return { classificacao: 'Fraco', cor: 'var(--color-primary-blue)' };
};

const getMarchaClassificacao = (passos: number) => {
  if (passos >= 400) return { classificacao: 'Excelente', cor: 'var(--color-primary-green)' };
  if (passos >= 300) return { classificacao: 'Bom', cor: 'var(--color-primary-green)' };
  if (passos >= 200) return { classificacao: 'Regular', cor: 'var(--color-primary-blue)' };
  return { classificacao: 'Fraco', cor: 'var(--color-primary-blue)' };
};

const getKatzClassificacao = (pontos: number) => {
  if (pontos === 6) return { classificacao: 'Independente', cor: 'var(--color-primary-green)' };
  if (pontos >= 4) return { classificacao: 'Dependência leve', cor: 'var(--color-primary-blue)' };
  if (pontos >= 2) return { classificacao: 'Dependência moderada', cor: 'var(--color-primary-blue)' };
  return { classificacao: 'Dependência severa', cor: 'var(--color-primary-blue)' };
};

const getLawtonClassificacao = (pontos: number) => {
  if (pontos >= 24) return { classificacao: 'Independente', cor: 'var(--color-primary-green)' };
  if (pontos >= 18) return { classificacao: 'Dependência leve', cor: 'var(--color-primary-blue)' };
  if (pontos >= 12) return { classificacao: 'Dependência moderada', cor: 'var(--color-primary-blue)' };
  return { classificacao: 'Dependência severa', cor: 'var(--color-primary-blue)' };
};

const getTugClassificacao = (segundos: number) => {
  if (segundos < 12) return { classificacao: 'Baixo risco', cor: 'var(--color-primary-green)' };
  if (segundos < 21) return { classificacao: 'Risco médio', cor: 'var(--color-primary-blue)' };
  return { classificacao: 'Alto risco', cor: 'var(--color-primary-blue)' };
};
...
export default function Home() {
  ...
  const generatePDF = async (data: AvaliacaoData) => {
    ...
    try {
      ...
      // Header
      pdf.setFillColor(26, 75, 159); // Azul Primário (1A4B9F em RGB)
      pdf.rect(0, 0, pageWidth, 30, 'F');
      
      pdf.setTextColor(0, 0, 0);
      pdf.setFontSize(18);
      pdf.setFont('helvetica', 'bold');
      pdf.text('SENHORAS & SENHORES - SAÚDE E BEM ESTAR', pageWidth / 2, 15, { align: 'center' });
      pdf.text('RELATÓRIO DE AVALIAÇÃO FÍSICO FUNCIONAL', pageWidth / 2, 25, { align: 'center' });
      ...
      pdf.setTextColor(imcClass.cor === 'var(--color-primary-green)' ? 56 : 26, imcClass.cor === 'var(--color-primary-green)' ? 176 : 75, imcClass.cor === 'var(--color-primary-green)' ? 74 : 159);
      ...
      pdf.setTextColor(mmi1Class.cor === 'var(--color-primary-green)' ? 56 : 26, mmi1Class.cor === 'var(--color-primary-green)' ? 176 : 75, mmi1Class.cor === 'var(--color-primary-green)' ? 74 : 159);
      pdf.text(`(${mmi1Class.classificacao})`, 60, yPosition);
      pdf.setTextColor(0, 0, 0);
      pdf.text(`2ª Tentativa: ${data.mmiTentativa2}s`, 120, yPosition);
      pdf.setTextColor(mmi2Class.cor === 'var(--color-primary-green)' ? 56 : 26, mmi2Class.cor === 'var(--color-primary-green)' ? 176 : 75, mmi2Class.cor === 'var(--color-primary-green)' ? 74 : 159);
      ...
      pdf.setTextColor(pd1Class.cor === 'var(--color-primary-green)' ? 56 : 26, pd1Class.cor === 'var(--color-primary-green)' ? 176 : 75, pd1Class.cor === 'var(--color-primary-green)' ? 74 : 159);
      pdf.text(`(${pd1Class.classificacao})`, 70, yPosition);
      pdf.setTextColor(0, 0, 0);
      pdf.text(`2ª: ${data.preensaoDireita2}kg`, 120, yPosition);
      pdf.setTextColor(pd2Class.cor === 'var(--color-primary-green)' ? 56 : 26, pd2Class.cor === 'var(--color-primary-green)' ? 176 : 75, pd2Class.cor === 'var(--color-primary-green)' ? 74 : 159);
      pdf.text(`(${pd2Class.classificacao})`, 150, yPosition);
      pdf.setTextColor(0, 0, 0);
      yPosition += 6;
      
      pdf.text(`Mão Esquerda - 1ª: ${data.preensaoEsquerda1}kg`, 15, yPosition);
      pdf.setTextColor(pe1Class.cor === 'var(--color-primary-green)' ? 56 : 26, pe1Class.cor === 'var(--color-primary-green)' ? 176 : 75, pe1Class.cor === 'var(--color-primary-green)' ? 74 : 159);
      pdf.text(`(${pe1Class.classificacao})`, 70, yPosition);
      pdf.setTextColor(0, 0, 0);
      pdf.text(`2ª: ${data.preensaoEsquerda2}kg`, 120, yPosition);
      pdf.setTextColor(pe2Class.cor === 'var(--color-primary-green)' ? 56 : 26, pe2Class.cor === 'var(--color-primary-green)' ? 176 : 75, pe2Class.cor === 'var(--color-primary-green)' ? 74 : 159);
      ...
      pdf.setTextColor(marchaClass.cor === 'var(--color-primary-green)' ? 56 : 26, marchaClass.cor === 'var(--color-primary-green)' ? 176 : 75, marchaClass.cor === 'var(--color-primary-green)' ? 74 : 159);
      ...
      pdf.setTextColor(katzClass.cor === 'var(--color-primary-green)' ? 56 : 26, katzClass.cor === 'var(--color-primary-green)' ? 176 : 75, katzClass.cor === 'var(--color-primary-green)' ? 74 : 159);
      pdf.text(`(${katzClass.classificacao})`, 60, yPosition);
      pdf.setTextColor(0, 0, 0);
      pdf.text(`LAWTON: ${data.lawtonPontos} pontos`, 120, yPosition);
      pdf.setTextColor(lawtonClass.cor === 'var(--color-primary-green)' ? 56 : 26, lawtonClass.cor === 'var(--color-primary-green)' ? 176 : 75, lawtonClass.cor === 'var(--color-primary-green)' ? 74 : 159);
